...
class Admin extends CI_Controller {
    ...
    public function tambah_admin()
    {
        if ($this->session->userdata('logged_in') == true AND $this->session->userdata('id_role') == 3) {
			...
			$alamat = $this->input->post('alamat'); // Mengambil data alamat dari input
			$telepon = $this->input->post('telepon'); // Mengambil data telepon dari input
			$jenis_kelamin = $this->input->post('jenis_kelamin'); // Mengambil data tanggal lahir dari input

            $id_role = 2;
            $id = md5($username . $email . $password);

            $hasil = $this->m_user->pendaftaran_user($id, $username, $email, $password, $id_role, $nama, $alamat, $telepon, $jenis_kelamin);
    ...
    public function edit_admin()
    {
        if ($this->session->userdata('logged_in') == true AND $this->session->userdata('id_role') == 3) {
            ...
            $hasil = $this->m_user->update_user($id_user, $username, $password);
